...
def compute_new_worry(initial_worry, worries):

    if worries[0] == '+':
        if worries[1].isdigit():
            return (int(initial_worry) + int(worries[1])) // 3
        else:
            return (int(initial_worry) + int(initial_worry)) // 3
    elif worries[0] == '*':
        if worries[1].isdigit():
            return (int(initial_worry) * int(worries[1])) // 3
        else:
    ...
    for round in range(20):
        for m in monkeys:
            for i, item in enumerate(monkeys[m]['items']):
                new_worry = compute_new_worry(item, monkeys[m]['worry'])
